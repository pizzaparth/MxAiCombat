"""
Segmentation Validation Script — v19
Hardware: Intel Core 5 210H, 16 GB RAM, 4 GB VRAM

Fixes vs v18:
  - Unified IoU metric: same compute_iou / compute_iou_per_class functions
    used in training (was previously a different implementation, causing
    train vs test IoU to appear inconsistent even on the same data).
  - IoU is now computed per-image and averaged (image-level mIoU),
    matching how val IoU is reported during training.
  - Added per-image IoU histogram saved to output_dir for debugging
    which images the model struggles on.
"""
...
# ============================================================================
# Metrics — unified with training script
# ============================================================================

def compute_iou(pred_classes, target, num_classes=10):
    """
    pred_classes: (H, W) or flattened — argmaxed predictions (not logits).
    Returns scalar mean IoU over classes present in this sample.
    """
...
def compute_iou_per_class(pred_classes, target, num_classes=10):
    """Returns (mean_iou, [per_class_iou]) for one image or batch."""
    p = pred_classes.view(-1)
    t = target.view(-1)
    per_class = []
    for c in range(num_classes):
...
def save_metrics_summary(results, output_dir):
    ...
    # Per-image IoU histogram (useful for debugging hard images)
    if 'per_image_iou' in results and len(results['per_image_iou']) > 0:
...
def main():
    ...
    parser.add_argument('--hidden_dim', type=int, default=256,
                        help='Must match hidden_dim used during training')
    ...
    # Resolution — must match training exactly
    ...
    # ── Inference ─────────────────────────────────────────────────────────────
    # Per-image accumulators (matches how training val IoU is computed)
    per_image_iou    = []
    per_image_acc    = []
    all_class_iou    = []
    sample_count     = 0
    ...
    with torch.no_grad():
        ...
        for imgs, labels, data_ids in pbar:
            ...
            # Compute metrics per image (not per batch) — matches training eval
            for i in range(imgs.shape[0]):
                img_iou, class_iou = compute_iou_per_class(preds[i], labels[i])
            ...
            batch_iou = float(np.mean(per_image_iou[-imgs.shape[0]:]))
            pbar.set_postfix(iou=f"{batch_iou:.3f}")
